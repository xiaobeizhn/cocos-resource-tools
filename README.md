...
- 🔍 **依赖扫描**：通过 BFS 递归收集指定 Prefab 文件夹内所有 Prefab 的直接与间接依赖，精准定位未引用资源
- 📂 **路径自动补全**：输入框支持项目内文件夹路径自动补全，快速选择目标目录
- ✅ **批量选择删除**：支持全选 / 手动勾选，一键批量删除未使用资源
...
2. 在面板中填写：
   - **Prefab 文件夹**：存放 Prefab 的目录路径（如 `db://assets/prefabs`）
